      ...
      try {
        ...
        usersSnapshot.forEach((docSnap) => {
          if (!assignedIds.has(docSnap.id)) {
            ...
            <button class="delete-btn" data-uid="${docSnap.id}">Eliminar asignaci\xF3n</button>
          `;
            li.querySelector(".assign-btn").addEventListener("click", async () => {
              try {
                await setDoc(
                  doc(db, `trainers/${currentTrainer.uid}/clients`, docSnap.id),
                  { ...userData, assignedAt: /* @__PURE__ */ new Date() }
                );
                await loadUnassignedUsers();
              } catch (error) {
            ...
            li.querySelector(".delete-btn").addEventListener("click", async () => {
              try {
                ...
                console.error("Error eliminando asignaci\xF3n:", error);
