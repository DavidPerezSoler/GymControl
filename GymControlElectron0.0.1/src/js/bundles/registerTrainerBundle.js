...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  // src/js/registerTrainer.js
  var app = initializeApp(firebaseConfig);
  var auth = getAuth(app);
  var db = getFirestore(app);
  var form = document.getElementById("reg-form");
  var errMsg = document.getElementById("reg-error");
  form.addEventListener("submit", async (e) => {
    e.preventDefault();
    errMsg.textContent = "";
    const email = document.getElementById("new-email").value.trim();
    const pass = document.getElementById("new-password").value.trim();
    const name4 = document.getElementById("new-name").value.trim();
    try {
      const secondaryApp = initializeApp(firebaseConfig, "Secondary");
      const secondaryAuth = getAuth(secondaryApp);
      const res = await createUserWithEmailAndPassword(secondaryAuth, email, pass);
      const uid = res.user.uid;
      await setDoc(doc(db, "trainers", uid), { name: name4, email });
      alert("Entrenador creado \u{1F389}");
      window.close();
      await signOut(secondaryAuth);
    } catch (e2) {
      console.error(e2);
      errMsg.textContent = "Error: " + e2.message;
    }
  });
})();
