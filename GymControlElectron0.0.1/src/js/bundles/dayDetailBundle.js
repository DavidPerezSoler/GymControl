...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  window.api.onDayDetailLoad(async ({ clientUid, date }) => {
    ...
    <div class="modal-content">
      <h3>\xBFCu\xE1ntos sets?</h3>
      <input type="number" id="modal-sets-input" min="1" placeholder="N\xBA de sets" />
  ...
  `;
    ...
    async function loadAssigned() {
      ...
      try {
        ...
        assigned = snap.exists() && Array.isArray(snap.data().exercises) ? snap.data().exercises : [];
      } catch (err) {
        console.error(err);
      }
    ...
    async function loadAvailable() {
      ...
      all.filter(
        (ex) => ex.name.toLowerCase().includes(s) && (!m || ex.primaryMuscles.join(",").toLowerCase().includes(m)) && (!l || ex.level === l)
      ).forEach((ex) => {
        const li = document.createElement("li");
        ...
        li.querySelector(".view-detail").addEventListener("click", () => window.api.openDetail(ex));
        li.querySelector(".assign-btn").addEventListener("click", () => {
          ...
          btnSetsOk.onclick = async () => {
            ...
            saveBtn.onclick = async () => {
              ...
              await updateDoc(dateRef, { exercises: arrayUnion({ name: ex.name, sets: setsData }) }).catch(async () => setDoc(dateRef, { exercises: [{ name: ex.name, sets: setsData }] }));
              loadAssigned();
            };
          };
        });
        li.querySelector(".edit-api").addEventListener("click", async () => {
          ...
          btnExSave.onclick = async () => {
            ...
            data.primaryMuscles = exFields.primary.value.split(",").map((s2) => s2.trim());
            data.secondaryMuscles = exFields.secondary.value.split(",").map((s2) => s2.trim());
        ...
        li.querySelector(".delete-api").addEventListener("click", async () => {
          if (!confirm(`\xBFBorrar "${ex.name}"?`)) return;
    ...
    [searchInput, filterMuscle, filterLevel].forEach(
      (el) => el.addEventListener("input", loadAvailable)
    );
